...
$Button1 = GUICtrlCreateButton("Button1", 208, 368, 177, 49)
$List1 = GUICtrlCreateList("", 24, 8, 577, 344)

GUISetState(@SW_SHOW)
#EndRegion ### END Koda GUI section ###
GUICtrlSetData($List1, "1")
GUICtrlSetData($List1, "file")
GUICtrlSetData($List1, "azaza")
...
While 1
	...
	Switch $nMsg
		Case $Button1
			$Path = FileSelectFolder('Выберите папку', '', 7, @DesktopDir, $Form1)
			If @error Or Not StringInStr(FileGetAttrib($Path), "D") Then
				MsgBox(4096, 'Ошибка выбора папки', $Path)
				ContinueLoop
			EndIf
            ;MsgBox(4096, 'Выбрана папка', $Path)
			$finded_array = SearchImages($Path)
			if IsArray($finded_array) Then
				;_ArrayDisplay($finded_array)
				GUICtrlSetData($List1, "")
				For $i = 1 to UBound($finded_array) - 1
					GUICtrlSetData($List1, $finded_array[$i])
				Next
...
Func SearchImages($folder = '')
If $folder = '' Then return 0
$FileList = _FO_FileSearch($folder,'jpg|jpeg|gif|png|bmp', True, 25, 1, 1, 2)
