...
Func Example()
	...
	$g_hListBox = GUICtrlCreateList("", 2, 2, 396, 296)
    GUISetState(@SW_SHOW)
            ...
            Switch $iCode
                Case $LBN_DBLCLK ; Sent when the user double-clicks a string in a list box
                    _DebugPrint("$LBN_DBLCLK" & @CRLF & "--> hWndFrom:" & @TAB & $hWndFrom & @CRLF & _
                            ...
                            "-->Code:" & @TAB & $iCode & @CRLF )
							;"-->Value:" & @TAB & $g_hListBox[$iCode])
							;if IsArray($hWndListBox) Then _ArrayDisplay($hWndListBox)
							MsgBox(64, "name", GUICtrlRead($g_hListBox))
                    ; no return value
